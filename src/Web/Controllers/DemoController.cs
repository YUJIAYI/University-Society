...
namespace Web.Controllers
{
    public class DemoController : BaseController
    {
        ...
        private IRepository<demo_dayoff> _dayoff;
        ...
        // /Demo/TeacherCheck
        public ActionResult GetDayOffInfo(string id)
        {
            return Json(State.Success, _dayoff.Find(id));
        }
        ...
        public ActionResult TestWorkFlow()
        {
            ...
            const string DEMO_LoginedUser = "1325112033";
            var DEMO_RoleList = new List<string>()
            {
                "3NRBC9DXVW"
            };
            //----------创建实例
            var instance = _workFlowService.CreateInstance(DEMO_WORKFLOWID, DEMO_OPERATOR);
            var instanceId = instance.Instance.WorkFlowInstanceID;
            //获取待办
            var toDo = _workFlowService.GetToDo(DEMO_LoginedUser, DEMO_RoleList);
            //查看下一步的任务
            ...
            toDo = _workFlowService.GetToDo( DEMO_LoginedUser, DEMO_RoleList);
           ...
           // var resultE= _workFlowService.MoveNext(instance, new { }, DEMO_OPERATOR);
            return Alert("测试成功！");
        }    // /Demo/TestOrder
        ...
        //demo/AjaxForm
        public ActionResult AjaxForm(string id)
        {
            /*
             {"name":"请输入姓名","sex":"aaaa",
             "time":"2010-2-11 11：00","date":"2010-2-11",
             "desc":"请输入简介","winnerfruit":"1",
             "summerfriut":"3","autologin":"on","note":""}
             */
            return Json(State.Success,new
            {
                name="张三",
                sex = "1",
                time = DateTime.Now,
